class VisionKronoEvents {
    ...
    async initSupabase() {
        try {
            ...
            // Verificar se já existe instância global do Supabase
            if (window.supabaseClient && window.supabaseClient.supabase && window.supabaseClient.isConnected && window.supabaseClient.initialized) {
                ...
                console.log('🔍 this.supabaseClient definido:', !!this.supabaseClient);
                return;
            }
            ...
            console.log('🔍 Estado atual:', {
                ...
                isConnected: window.supabaseClient?.isConnected,
                initialized: window.supabaseClient?.initialized
            });
            
            await new Promise(resolve => setTimeout(resolve, 1000));
            
            console.log('🔍 Estado após aguardar:', {
                supabaseClient: !!window.supabaseClient,
                supabase: !!window.supabaseClient?.supabase,
                isConnected: window.supabaseClient?.isConnected,
                initialized: window.supabaseClient?.initialized
            ...
            if (window.supabaseClient && window.supabaseClient.supabase && window.supabaseClient.isConnected && window.supabaseClient.initialized) {
                ...
                console.warn('⚠️ Instância global do Supabase não encontrada - usando fallback');
            ...
            console.log('📊 Supabase inicializado:', !!this.supabaseClient);
            console.log('🔍 Finalizando initSupabase...');
    ...
    async loadEvents() {
        try {
            ...
            if (!this.supabaseClient || !this.supabaseClient.supabase) {
                console.warn('⚠️ Supabase não disponível - mostrando mensagem');
                this.eventsGrid.innerHTML = '<div class="loading">❌ Supabase não disponível</div>';
                return;
            }
...
// Inicializar apenas na página de eventos
