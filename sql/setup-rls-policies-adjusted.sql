...
WITH CHECK (
    EXISTS (
        ...
        JOIN events e ON e.id = event_configurations.event_id
        WHERE up.user_id = auth.uid()
            AND up.profile_type IN ('admin', 'event_manager')
            AND (
                up.profile_type = 'admin' 
                OR up.organizer_id = e.organizer_id
            )
...
BEGIN
    ...
    RAISE NOTICE '  - calibrations: 4 policies (read, insert, update, delete)';
    RAISE NOTICE '';
