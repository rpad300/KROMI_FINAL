-- Script corrigido para configurações de processador por evento
-- Execute este script no Supabase SQL Editor

-- 1. Remover funções existentes que podem ter conflito
DROP FUNCTION IF EXISTS get_event_processor_setting(UUID);
DROP FUNCTION IF EXISTS set_event_processor_setting(UUID, TEXT, TEXT, DECIMAL, BOOLEAN);
...
-- 5. Criar função para salvar configurações
...
-- 6. Criar função para buscar configurações
...
-- 7. Testar com evento existente
DO $$
DECLARE
    test_event_id UUID;
BEGIN
    -- Buscar ID do evento teste1
    SELECT id INTO test_event_id FROM events WHERE name = 'teste1' LIMIT 1;
    
    IF test_event_id IS NOT NULL THEN
        -- Testar inserção
        PERFORM set_event_processor_setting(test_event_id, 'gemini', 'balanced', 0.8, true);
        RAISE NOTICE 'Configuração salva para evento: %', test_event_id;
    ELSE
        RAISE NOTICE 'Evento teste1 não encontrado';
    END IF;
END $$;
