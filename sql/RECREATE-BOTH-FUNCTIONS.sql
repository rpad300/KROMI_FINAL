-- ============================================================================
-- Recriar AMBAS as funções (process_device_detection E process_pending_detections)
-- ============================================================================

-- DROP ambas
DROP FUNCTION IF EXISTS process_device_detection(UUID) CASCADE;
DROP FUNCTION IF EXISTS process_pending_detections(INTEGER) CASCADE;

-- 1. CRIAR process_device_detection
...
BEGIN
    ...
    BEGIN
        ...
        IF v_has_dorsal THEN
            ...
            RETURN json_build_object('success', true, 'action', 'direct_detection', 'detection_id', v_detection_result_id);
        ELSE
            ...
            RETURN json_build_object('success', true, 'action', 'sent_to_buffer', 'buffer_id', v_buffer_result_id);
        END IF;
...
-- 2. CRIAR process_pending_detections
CREATE FUNCTION process_pending_detections(p_batch_size INTEGER DEFAULT 10)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
