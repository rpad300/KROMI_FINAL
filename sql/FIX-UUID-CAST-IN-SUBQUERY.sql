-- ============================================================================
-- KROMI - Fix UUID com CAST EXPLÍCITO NA SUBQUERY
-- ============================================================================
-- Força CAST ::UUID dentro da própria subquery
-- ============================================================================
...
BEGIN
    ...
    -- UPDATEs SEPARADOS com CAST EXPLÍCITO NA SUBQUERY
    -- UPDATE 1: event_id com CAST na subquery
    ...
    SET event_id = (
        ...
        WHERE ed.access_code = v_detection.access_code 
        LIMIT 1
    )::UUID  -- CAST adicional na saída da subquery
    WHERE id = p_detection_id;
    
    -- UPDATE 2: device_id com CAST na subquery
    UPDATE device_detections
    SET device_id = (
        SELECT ed.device_id::UUID 
        FROM event_devices ed 
        WHERE ed.access_code = v_detection.access_code 
        LIMIT 1
    )::UUID  -- CAST adicional na saída da subquery
...
-- ============================================================================
-- ✅ CORREÇÃO APLICADA - CAST DUPLO (na subquery E na saída)
-- ============================================================================
-- Esta versão força CAST ::UUID em DOIS pontos:
-- 1. Dentro da subquery (ed.device_id::UUID)
-- 2. Na saída da subquery ((SELECT...)::UUID)
-- ============================================================================
